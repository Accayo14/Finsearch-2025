...
def train_dqn(episodes=600):
    ...
    for i_episode in range(episodes):
        ...
        if i_episode % 50 == 0:
            avg_duration = np.mean(episode_durations[-50:]) if len(episode_durations) >= 50 else np.mean(episode_durations)
            print(f'Episode {i_episode}, Average Duration: {avg_duration:.2f}')
    
    return policy_net, episode_durations

def test_dqn(model, episodes=10):
...
if __name__ == "__main__":
    ...
    trained_agent, training_scores = train_dqn(episodes=500)
    
    print(f"Training completed!")
    print(f"Final average score: {np.mean(training_scores[-100:]):.2f}")
    
    print("\nTesting trained agent...")
    test_scores = test_dqn(trained_agent, episodes=5)
    ...
    if len(training_scores) >= 100:
        running_avg = [np.mean(training_scores[max(0, i-100):i+1]) for i in range(len(training_scores))]
